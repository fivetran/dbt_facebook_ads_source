...
final as (
    ...
    select
        ...
        timezone_name,
        case 
            when id is null then row_number() over (partition by source_relation, id order by source_relation) = 1 
            else row_number() over (partition by source_relation order by _fivetran_synced desc)
